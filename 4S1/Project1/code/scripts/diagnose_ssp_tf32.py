...
for j,i in enumerate(items):
 ...
 with torch.inference_mode():embedding=model.sam.image_encoder(sam_img).float(); torch.backends.cuda.matmul.allow_tf32=True; half_embedding=model.sam.image_encoder(sam_img).float(); torch.backends.cuda.matmul.allow_tf32=False; print('EMBED',float((embedding-half_embedding).abs().max()),float((embedding-half_embedding).abs().mean()),bool(torch.isfinite(half_embedding).all()),flush=True)
