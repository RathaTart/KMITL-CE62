...
ROOT=Path('/home/osta/lisa-eval');OUT=ROOT/'code/results/router_confirm_20260915';OUT.mkdir(exist_ok=True); assert not any(OUT.iterdir()), 'Output must be empty'
...
rng=random.Random(2026091507);used=set();selected=[]
...
items.sort(key=lambda i:i['image_id']);assert len({i['image_sha256'] for i in items})==90
write(OUT/'manifest.json',dict(dataset='grefcoco',dataset_label='Fresh gRefCOCO person subset',selection='90 unique images, official testA only, 30 single / 30 multi / 30 no-target; prior manifests and hashes excluded',status='Freeze methods using historical development before scoring this new subset',source='https://github.com/henghuiding/gRefCOCO',items=items))
