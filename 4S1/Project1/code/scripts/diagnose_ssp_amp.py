...
for j,i in enumerate(items):
 ...
 with torch.inference_mode():embedding=model.sam.image_encoder(sam_img).float(); half_embedding=amp_encode(model,sam_img); print('EMBED',float((embedding-half_embedding).abs().max()),float((embedding-half_embedding).abs().mean()),bool(torch.isfinite(half_embedding).all()),flush=True)
