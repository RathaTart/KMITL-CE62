...
rng=random.Random(2026091513);used=set();selected=[]
for kind in ['no_target','multi_target','single_in_crowd']:
    required=14 if kind=='no_target' else 30
...
items.sort(key=lambda i:i['image_id']);assert len({i['image_sha256'] for i in items})==74
write(OUT/'manifest.json',dict(dataset='grefcoco',dataset_label='Fresh gRefCOCO person subset',selection='74 unique images, official testA only, 30 single / 30 multi / 14 no-target; prior manifests and hashes excluded',status='Freeze methods using historical development before scoring this new subset',source='https://github.com/henghuiding/gRefCOCO',items=items))
