...
folder=a.out_dir/'p1_masks';folder.mkdir(exist_ok=True);rows=a.out_dir/'p1_rows.jsonl';done={r['key'] for r in [json.loads(line) for line in rows.read_text().splitlines()]} if rows.exists() else set()
