...
ROOT=Path('/home/osta/lisa-eval');OUT=ROOT/'code/results/training_free_fresh_20260914';OUT.mkdir(exist_ok=True); assert not any(OUT.iterdir()), 'Output must be empty'
...
rng=random.Random(2026091402);used=set();selected=[]
...
items.sort(key=lambda i:i['image_id']);assert len({i['image_sha256'] for i in items})==60
write(OUT/'manifest.json',dict(dataset='grefcoco',dataset_label='Fresh gRefCOCO person subset',selection='60 unique images, official testA only, 20 single / 20 multi / 20 no-target; prior manifests and hashes excluded',status='Freeze methods using historical development before scoring this new subset',source='https://github.com/henghuiding/gRefCOCO',items=items))
