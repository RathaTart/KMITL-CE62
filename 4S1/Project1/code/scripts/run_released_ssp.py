...
model=model.cuda().eval();model.sam.image_encoder.half();load_s=time.perf_counter()-start
...
for j,i in enumerate(items):
 ...
 image_mask=target['mask'].unsqueeze(0).cuda();td={k:v.unsqueeze(0).cuda() for k,v in target.items() if isinstance(v,torch.Tensor)};words=tokenize([i['expression'].lower()+'.']).cuda();wm=words!=0;sam_img=sam_transform(np.asarray(original)).unsqueeze(0).cuda().half();img=img.unsqueeze(0).cuda()
 torch.cuda.synchronize();tm=time.perf_counter()
 with torch.inference_mode():embedding=model.sam.image_encoder(sam_img).float()
