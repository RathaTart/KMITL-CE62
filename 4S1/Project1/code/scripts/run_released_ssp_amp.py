...
p=argparse.ArgumentParser();p.add_argument('--limit',type=int,default=90);p.add_argument('--out',default='released_checkpoints_20260915/SSP_SAM_224_AMP');a=p.parse_args();O=C/'results'/a.out;O.mkdir(parents=True,exist_ok=True);(O/'masks').mkdir(exist_ok=True);(O/'native_masks').mkdir(exist_ok=True)
...
transforms=[RandomResize([224],record_resize_info=True),ToTensor(keys=[]),NormalizeAndPad(size=224,center_place=True)];sam_transform=SAMResize();items=json.loads((C/'results/router_confirm_20260915/manifest.json').read_text())['items'][:a.limit]
def amp_encode(model,sam_img):
 with torch.autocast('cuda',dtype=torch.float16):return model.sam.image_encoder(sam_img).float()
for j,i in enumerate(items):
 ...
 with torch.inference_mode():embedding=amp_encode(model,sam_img)
 torch.cuda.synchronize();encoder_s=time.perf_counter()-tm;tm=time.perf_counter()
 with torch.inference_mode():out=model(img,image_mask,words,wm,embedding,td['ori_size'],td)['pred_masks']
