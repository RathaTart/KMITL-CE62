...
Turacc.basket.add_pizza(controller.create_pizza_item("pepperoni", "L", 2))
Turacc.basket.add_pizza(controller.create_pizza_item("margherita", "M", 0))
...
Turacc.create_order()
Turacc.uncompleted_order.apply_coupon(controller.search_coupon_by_code("ABC123"))
Turacc.uncompleted_order.create_payment("cash")
...
@app.post('/basket/{user_id}/add_pizza', tags=["Basket"])
def add_pizza_to_basket(face: str, size: str, quantity: int, req: Request) -> dict:
    try:
        ...
        account = controller.search_account_by_name(user_id)
        account.basket.add_pizza(controller.create_pizza_item(face, size, quantity))
        return {"message": f"pizza added to basket for user {user_id}"}
...
@app.post('/basket/{user_id}/edit_pizza', tags=["Basket"], response_model=None)
def edit_pizza_in_basket(user_id: str, face:str, size:str, quantity:str, new_size: str, new_quantity: int) -> dict:
    if new_quantity > 0:    
        account = controller.search_account_by_user_id(user_id)
        item = controller.create_pizza_item(face, size, quantity)
        if item in account.basket.pizza_list:
            account.basket.edit_pizza(item, new_size, new_quantity)
            return {"message": "Pizza edited in basket"}
        else:
            return {"message": "Basket not found"}
    else:
         return {"message": "Pls enter quantity more than 0"}      
# @app.post('/basket/{user_id}/edit_drink', tags=["Basket"])
# def edit_drink_in_basket(user_id: str, item: Drink_item, new_size: str, new_quantity: int) -> dict:
#     account = controller.search_account_by_user_id(user_id)
#     if account:
#         if account.basket:
#             account.basket.edit_drink(item, new_size, new_quantity)
#             return {"message": "Drink edited in basket"}
#         else:
#             return {"message": "Basket not found"}
#     else:
#         return {"message": "User not found"}

@app.post('/basket/{user_id}/remove_pizza', tags=["Basket"])
def remove_pizza_from_basket(user_id: str, face: str, size: str, quantity: int) -> dict:
    account = controller.search_account_by_user_id(user_id)
    if account:
        if account.basket:
            account.basket.remove_pizza(controller.create_pizza_item(face, size, quantity))
            return {"message": "Pizza removed from basket"}
        else:
            return {"message": "Basket not found"}
    else:
        return {"message": "User not found"}
    
@app.post('/basket/{user_id}/remove_drink', tags=["Basket"])
def remove_drink_from_basket(user_id: str, name: str, size: str, quantity: int) -> dict:
    account = controller.search_account_by_user_id(user_id)
    if account:
        if account.basket:
            account.basket.remove_drink(controller.create_drink_item(name, size, quantity))
            return {"message": "Drink removed from basket"}
        else:
            return {"message": "Basket not found"}
    else:
        return {"message": "User not found"}
    ...
    try:
        ...
        if account:
            if account.basket:
                ...
                drink_list = [drink_item for drink_item in account.basket.drink_list]
...
# @app.get('/basket/payment/', tags=["Basket Cost"])
# def get_basket_cost(req: Request):
#     try:
#         payload = jwt.decode(req.headers['authentication'], SECRET_KEY, algorithms=[ALGORITHM])
#         userId = payload.get("sub")
#         if userId is None:
...
#         account = controller.search_account_by_user_id(userId)
#         if account:
#             if account.basket:
#                 return {"total_cost": account.basket.get_total_price()}
#             else:
#                 return {"message": "Basket not found"}
#         else:
